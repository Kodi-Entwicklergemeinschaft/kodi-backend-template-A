...
async function bootstrap() {
  ...
  const swaggerTitle = `Users Service | ${swaggerConfig.title || 'KODI Microservices API'}`;
  const serverUrl = getSwaggerServerUrl(configService, 'users');

  const documentBuilder = new DocumentBuilder()
    .setTitle(swaggerTitle)
    .setDescription(swaggerConfig.description || 'API documentation for KODI Users Service')
