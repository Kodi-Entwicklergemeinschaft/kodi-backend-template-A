...
async function bootstrap() {
  ...
  app.connectMicroservice<MicroserviceOptions>(getRmqConsumerOptions(configService, 'admin'));
  ...
  const swaggerTitle = `Admin Service | ${swaggerConfig.title || 'KODI Microservices API'}`;
  const serverUrl = getSwaggerServerUrl(configService, 'admin');

  const documentBuilder = new DocumentBuilder()
    .setTitle(swaggerTitle)
    .setDescription(swaggerConfig.description || 'API documentation for KODI Admin Service')
