...
async function bootstrap() {
  ...
  const swaggerTitle = `Terminal Service | ${swaggerConfig.title || 'KODI Microservices API'}`;
  const serverUrl = getSwaggerServerUrl(configService, 'terminal');

  const documentBuilder = new DocumentBuilder()
    .setTitle(swaggerTitle)
    .setDescription(swaggerConfig.description || 'API documentation for KODI Terminal Service')
