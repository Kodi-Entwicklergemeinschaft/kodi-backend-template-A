...
// One-time cache-bust param for raster images only (not SVG icons).
// After running the script once, this can be removed (runtime uploads
// rely on Cache-Control: no-cache set on the S3 object).
const CACHE_BUST_VERSION = `v=${Date.now()}`;

// Generate public URL (clean, no query params)
...
// Generate public URL with cache-bust param (for raster images only)
function generateCacheBustedUrl(bucket: string, key: string): string {
  return `${generatePublicUrl(bucket, key)}?${CACHE_BUST_VERSION}`;
}
...
async function uploadCategoryImage(
  ...
  return generateCacheBustedUrl(bucket, key);
}
...
async function uploadCategoryIcon(
  ...
  const body = isSvg ? fileBuffer : await processImage(fileBuffer);
  const key = `categories/${categoryId}/icon${isSvg ? '.svg' : '.webp'}`;
  ...
  return isSvg ? generatePublicUrl(bucket, key) : generateCacheBustedUrl(bucket, key);
}
...
async function main() {
  ...
  for (const [cityName, cityData] of Object.entries(CITY_HEADER_IMAGE)) {
    ...
    try {
      // Find city by name (assuming Kodi for now, can be extended)
      const city = await cityPrisma.city.findFirst({
        where: {
          name: cityName === 'kodi' ? 'Kodi' : cityName,
        },
